...
// Note: L'import de useLocation et HeroCarousel suppose une configuration de projet
// avec react-router-dom et un composant HeroCarousel disponible.
// Pour un exemple autonome, ces lignes seraient commentées ou supprimées.
...
{
  // const location = useLocation(); // Décommenter si react-router-dom est utilisé
  // const params = useMemo(() => new URLSearchParams(location.search), [location.search]);
  // const forfaitParam = (params.get("forfait") || "").toLowerCase().trim();
  const forfaitParam = ""; // Valeur par défaut pour l'exemple
  ...
  return (
    <div className="min-h-screen bg-rosa-ivory">
      {/* ✅ Le HeroCarousel est commenté pour l'exemple autonome */}
      {/* <HeroCarousel /> */}
      <div className="py-8 text-center text-4xl font-playfair font-bold text-rosa-warm-gray">La Perle de Velours</div>
          ...
          <div>
            <div className="bg-white rounded-2xl shadow-lg p-8">
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {/* --- Bouton de validation ajouté --- */}
                <button
                  type="submit"
                  disabled={isSubmitting}
                  className="w-full bg-green-600 text-white py-4 px-6 rounded-xl font-inter font-medium hover:bg-green-700 focus:ring-2 focus:ring-green-500/50 focus:ring-offset-2 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center space-x-2 min-h-[48px]"
                >
                  {isSubmitting ? (
                    <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white" />
                  ) : (
