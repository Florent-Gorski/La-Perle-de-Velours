...
const Services: React.FC = () =>
{
  const services = [
    {
      icon: <Heart className="w-8 h-8 md:w-12 md:h-12 text-perle-honey" />,
      category: 'Manucure',
      id: 'manucure',
      services: [
    ...
    {
      icon: <Sparkles className="w-8 h-8 md:w-12 md:h-12 text-perle-honey" />,
      category: 'Pédicure',
      id: 'pedicure',
      services: [
    ...
    {
      icon: <Flower className="w-8 h-8 md:w-12 md:h-12 text-perle-honey" />,
      category: 'Esthétique',
      id: 'soins-visage',
      services: [
    ...
    {
      icon: <Zap className="w-8 h-8 md:w-12 md:h-12 text-perle-honey" />,
          ...
          <div className="space-y-12 md:space-y-20">
            {services.map((category, index) => (
              <div key={index} id={category.id} className="bg-white rounded-3xl shadow-lg overflow-hidden">
                <div className="bg-gradient-to-r from-perle-soft-beige to-perle-beige p-6 md:p-8">
                  <div className="flex items-center justify-center mb-4 md:mb-6">
                    {category.icon}
                  </div>
                  <h2 className="text-2xl md:text-3xl font-playfair font-bold text-perle-warm-gray text-center mb-2 md:mb-4">
                    {category.category}
                  </h2>
                </div>
                ...
                <div className="p-4 md:p-8">
                  <div className="space-y-4 md:grid md:grid-cols-2 md:gap-6 md:space-y-0">
                    {category.services.map((service, serviceIndex) => (
                      <div
                        key={serviceIndex}
                        className="border border-perle-beige/50 rounded-xl p-4 md:p-6 hover:shadow-lg hover:border-perle-honey/30 hover:bg-perle-soft-beige/50 transition-all duration-300"
                      >
                        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-3 gap-2">
                          <h3 className="text-base md:text-lg font-playfair font-bold text-perle-warm-gray">{service.name}</h3>
                          <span className="bg-perle-honey/20 text-perle-honey px-3 py-1 rounded-full text-sm font-inter font-medium self-start">
                            {service.duration}
                          </span>
                        </div>
                        <p className="text-perle-warm-gray/70 font-inter text-sm md:text-base leading-relaxed">{service.description}</p>
                      </div>
                    ))}
                  </div>
                </div>
      ...
      <section className="py-12 md:py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-8 md:mb-16">
            ...
            <p className="text-base md:text-xl font-inter text-perle-warm-gray/70">Réservation simple en 4 étapes</p>
          </div>
          ...
          <div className="space-y-6 md:grid md:grid-cols-2 lg:grid-cols-4 md:gap-8 md:space-y-0">
            {[
              ...
              <div key={index} className="text-center p-4 md:p-0">
                <div className="w-12 h-12 md:w-16 md:h-16 bg-perle-honey text-white rounded-full flex items-center justify-center text-lg md:text-2xl font-playfair font-bold mx-auto mb-4">
                  {item.step}
                </div>
                <h3 className="text-lg md:text-xl font-playfair font-bold text-perle-warm-gray mb-2">{item.title}</h3>
                <p className="text-perle-warm-gray/70 font-inter text-sm md:text-base">{item.description}</p>
              </div>
            ))}
          </div>
        </div>
